...
var matchWithCSVData = function(feedData) {
	...
	_.each(UNION_STATION_STOPS, function(current_stop) {
		...
		var justTheStationWeWant = _.each(filteredData, function(trippin) {
			...
			if(byRoute[routeMapId] && trippin.trip_update.stop_time_update.departure.time.low < byRoute[routeMapId].trip_update.stop_time_update.departure.time.low) {
				byRoute[routeMapId] = trippin;
			} else if(_.isNil(byRoute[routeMapId])) {
				byRoute[routeMapId] = trippin;
			}
		});
	});
