		...
		if(err) {
			...
			data.forEach(function(aTrip) {
				if(aTrip.trip_update.stop_time_update.departure && aTrip.trip_update.stop_time_update.departure.time.low) {
					...
					console.log("Route " + aTrip.trip_update.trip.route_id + " with trip id " + aTrip.trip_update.trip.trip_id + 
					" leaving from " + aTrip.trip_update.trip.stop_id_name + " toward " + aTrip.trip_update.trip.headsign + " direction " + aTrip.trip_update.trip.direction_id + " at " + displayDate.toTimeString());
				}
			});
		}
	});
